...
$offset = ($page - 1) * $limit;
$result = $conn->query("SELECT COUNT(*) AS total FROM bestsellers");
$total_books = $result->fetch_assoc()['total'];
$total_pages = ceil($total_books / $limit);
$sql = "SELECT * FROM bestsellers LIMIT $limit OFFSET $offset";
$result = $conn->query($sql);

if (isset($_GET['delete'])) {
    $id = (int)$_GET['delete'];
    $deleteQuery = "DELETE FROM bestsellers WHERE id = $id";
    if ($conn->query($deleteQuery) === TRUE) {
        echo "<script>alert('Book deleted successfully!'); window.location='bestseller.php';</script>";
    } else {
...
<body>
    ...
    <div class="content">
            ...
            <?php while ($row = $result->fetch_assoc()) { ?>
                <tr>
                    ...
                    <td class="action-buttons">
                        <a href="view_bestseller.php?id=<?= $row['id']; ?>" title="View">
                            <i class="fas fa-eye action-icon view-icon"></i>
                        </a>
                        <a href="edit_bestseller.php?id=<?= $row['id']; ?>" title="Edit">
        ...
        <button class="update-btn" onclick="window.location.href='add_bestseller.php'"><b>ADD BOOK</b></button>
    </div>
</body>
