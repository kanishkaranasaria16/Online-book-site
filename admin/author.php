...
$sql = "SELECT * FROM authors LIMIT $limit OFFSET $offset";
...
if (isset($_GET['delete'])) {
    ...
    if ($conn->query($deleteQuery) === TRUE) {
        ...
        echo "<script>alert('Error deleting author: " . $conn->error . "');</script>";
    }
}
?>
...
<div class="content">
    ...
    <table>
        ...
        <?php while ($row = $result->fetch_assoc()) { ?>
            <tr>
                <td><?= $row['id']; ?></td>
                <td><?= htmlspecialchars(mb_strimwidth($row['name'], 0, 50, '...')); ?></td>
                <?php
                ...
                <td><?= htmlspecialchars(mb_strimwidth($row['about'], 0, 50, '...')); ?></td>
                <td><?= htmlspecialchars(mb_strimwidth($row['description'], 0, 50, '...')); ?></td>
                <td class="action-buttons">
                    <a href="view_author.php?id=<?= $row['id']; ?>" title="View">
                        <i class="fas fa-eye action-icon view-icon"></i>
                    </a>
                    <a href="edit_author.php?id=<?= $row['id']; ?>" title="Edit">
                        <i class="fas fa-edit action-icon edit-icon"></i>
                    </a>
                    <a href="?delete=<?= $row['id']; ?>" onclick="return confirm('Are you sure you want to delete this author?');" title="Delete">
    ...
    <button class="update-btn" onclick="window.location.href='add_author.php'"><b>ADD AUTHOR</b></button>
</div>
</body>
